...
def process():
    ...
    df_lv = pd.concat([pd.read_excel(lv_file) for lv_file in glob.glob(f'{inpath}/id*.xlsx')])

    # ID Mapping
    df_id = pd.read_excel(glob.glob(f'{inpath}/isolate*.xlsx')[0], names=['Sample'], header=None)
    ...
    # get initial box_id
    #id_init=messagebox.askquestion(title='Format Master Sheet',
    #                    message='Do you want to also generate master-sheet-formatted file?')
    
    id_init = askinteger(title='Staring Box ID',prompt='Starts with box_id: ')
    ...
    # %% Export
    df_out.to_excel(f'{Path(inpath)}/CHOP_output.xlsx', index=False)
